...
docker run -v "$(pwd)":/tmp/touche-shared-task -v "$(pwd)"/output-dir:/tmp/output-dir -v "$(pwd)"/input-dir:/tmp/input-dir -v touche-shared-task-run:/tmp/touche-shared-task-run -d -t my-docker
id=$(sudo docker ps -aqf "name=my-docker")
docker exec -it "$id" ./tmp/touche-shared-task/run.sh